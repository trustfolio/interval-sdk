...
const IOPromise_1 = require("./IOPromise");
const IOError_1 = __importDefault(require("./IOError"));
const spreadsheet_1 = __importDefault(require("../components/spreadsheet"));
const displayTable_1 = __importDefault(require("../components/displayTable"));
const selectTable_1 = __importDefault(require("../components/selectTable"));
...
const displayGrid_1 = __importDefault(require("../components/displayGrid"));
const displayLink_1 = __importDefault(require("../components/displayLink"));
const displayImage_1 = __importDefault(require("../components/displayImage"));
...
class IOClient {
     ...
     */
    async renderComponents({ components, validator: groupValidator, choiceButtons, }) {
        if (this.isCanceled) {
            // Transaction is already canceled, host attempted more IO calls
            throw new IOError_1.default('TRANSACTION_CLOSED');
        }
        let validationErrorMessage;
        return new Promise(async (resolve, reject) => {
            const inputGroupKey = (0, uuid_1.v4)();
            ...
            const render = async () => {
                const packed = {
                    ...
                    toRender: components
                        ...
                        const { json, meta } = superjson_1.default.serialize((0, deserialize_1.stripUndefined)(props));
                        return {
                            ...renderInfo,
                            props: json,
                            propsMeta: meta,
            ...
            const inputGroupResponseHandler = async (result) => {
                try {
                    if (result.inputGroupKey &&
                    ...
                    if (result.kind === 'CANCELED') {
                        ...
                        reject(new IOError_1.default('CANCELED'));
                        return;
                    }
                    ...
                    if (result.valuesMeta) {
                        result.values = superjson_1.default.deserialize({
                            json: result.values,
                        ...
                        for (const [index, newState] of result.values.entries()) {
                            ...
                            if (newState &&
                                ...
                                try {
                                    ...
                                    this.logger.warn(`Error updating ${instance.methodName} component state, ignoring state update`, err);
                                }
                            }
                        }
                        render();
            ...
            const response = {
                returnValue: await Promise.all(components.map(comp => comp.returnValue)).then(returnValue => {
                    ...
                    if (!isReturned && !(choiceButtons === null || choiceButtons === void 0 ? void 0 : choiceButtons.length)) {
                        setChoice(undefined);
                    }
                    return returnValue;
                }),
     ...
     * ```
     */
    group(promises, props) {
        const promiseValues = Array.isArray(promises)
            ? promises
            : Object.values(promises);
        const exclusivePromises = promiseValues.filter(pi => pi instanceof IOPromise_1.ExclusiveIOPromise);
        ...
        const withChoicesPromises = promiseValues.filter(pi => pi instanceof IOPromise_1.WithChoicesIOPromise);
        if (withChoicesPromises.length > 0) {
            throw new __1.IntervalError(`Components with the following labels are chained with withChoices, which is not supported inside a group (call withChoices on the group itself instead): ${exclusivePromises
                .map(pi => pi.component.label)
                .join(', ')}`);
        }
        return new IOPromise_1.IOGroupPromise({
            ...
            continueButton: props === null || props === void 0 ? void 0 : props.continueButton,
        });
    }
    ...
    createExclusiveIOMethod(methodName, { componentDef, demoUnsupported = false, } = {}) {
        return (label, props) => {
            ...
            return new IOPromise_1.ExclusiveIOPromise({
                ...promiseProps,
                methodName,
                renderer: this.renderComponents.bind(this),
                label,
        ...
        return {
            ...
            input: {
                ...
                file: this.createIOMethod('UPLOAD_FILE', {
                    componentDef: (0, upload_1.file)(this.logger),
                }),
            },
            /**
            ...
            select: {
                ...
                table: this.createIOMethod('SELECT_TABLE', {
                    propsRequired: true,
                    componentDef: (0, selectTable_1.default)(this.logger),
                }),
            },
            /**
            ...
            display: {
                ...
                image: this.createIOMethod('DISPLAY_IMAGE', {
                    componentDef: displayImage_1.default,
                    propsRequired: true,
                }),
                /**
                ...
                table: this.createIOMethod('DISPLAY_TABLE', {
                    propsRequired: true,
                    componentDef: (0, displayTable_1.default)(this.logger),
                }),
                /**
                ...
                grid: this.createIOMethod('DISPLAY_GRID', {
                    propsRequired: true,
                    componentDef: displayGrid_1.default,
                }),
                /**
                ...
                video: this.createIOMethod('DISPLAY_VIDEO', {
                    componentDef: displayVideo_1.default,
                    propsRequired: true,
                }),
            },
            experimental: {
    ...
    onResponse(result) {
        var _a;
        const inputGroupKey = (_a = result.inputGroupKey) !== null && _a !== void 0 ? _a : this.previousInputGroupKey;
        if (!inputGroupKey) {
        ...
        if (!inputGroupHandler) {
            this.logger.error('No response handler defined for inputGroupKey', inputGroupKey);
            return;
        }
        try {
